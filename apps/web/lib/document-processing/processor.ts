...
import { distributeDocumentData } from "./auto-distribute";
import type {
  StoredDocument,
  StoredEmbedding,
  Equipement,
...
export async function processStoredDocument(
  ...
  // Stage 1.5: Automatically classify and distribute document data
  onProgress?.({
    stage: "extracting",
    progress: 15,
    message: "Classifying and extracting structured data...",
  });

  try {
    const distribution = await distributeDocumentData(
      documentId,
      textContent,
      document.fileName,
      userId
    );
    if (distribution.type !== "unknown") {
      console.log(
        `✅ Document classified as "${distribution.type}" - ${distribution.recordsCreated} record(s) created`
      );
    }
  } catch (error) {
    console.warn("Auto-distribution failed (non-critical):", error);
