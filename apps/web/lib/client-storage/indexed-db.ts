...
// ===== Structured Data Types (Automated Distribution) =====
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains(STORES.EQUIPEMENTS)) {
        const eqStore = db.createObjectStore(STORES.EQUIPEMENTS, { keyPath: "id" });
        eqStore.createIndex("userId", "userId", { unique: false });
        eqStore.createIndex("source_document_id", "source_document_id", { unique: false });
        eqStore.createIndex("Code_Equip", "Code_Equip", { unique: false });
      } else if (tx) {
        // Update existing store with missing indexes
        try {
          const eqStore = tx.objectStore(STORES.EQUIPEMENTS);
          if (!eqStore.indexNames.contains("source_document_id")) {
            eqStore.createIndex("source_document_id", "source_document_id", { unique: false });
          }
        } catch (e) {
          console.warn("Could not create index on existing EQUIPEMENTS store");
        }
      }

      // Maintenance store
      if (!db.objectStoreNames.contains(STORES.MAINTENANCE)) {
        const mStore = db.createObjectStore(STORES.MAINTENANCE, { keyPath: "id" });
        mStore.createIndex("userId", "userId", { unique: false });
        mStore.createIndex("source_document_id", "source_document_id", { unique: false });
        mStore.createIndex("Code_Equip", "Code_Equip", { unique: false });
      } else if (tx) {
        // Update existing store with missing indexes
        try {
          const mStore = tx.objectStore(STORES.MAINTENANCE);
          if (!mStore.indexNames.contains("source_document_id")) {
            mStore.createIndex("source_document_id", "source_document_id", { unique: false });
          }
        } catch (e) {
          console.warn("Could not create index on existing MAINTENANCE store");
        }
      }

      // Pieces de rechange store
      if (!db.objectStoreNames.contains(STORES.PIECES_RECHANGE)) {
        const pStore = db.createObjectStore(STORES.PIECES_RECHANGE, { keyPath: "id" });
        pStore.createIndex("userId", "userId", { unique: false });
        pStore.createIndex("source_document_id", "source_document_id", { unique: false });
      } else if (tx) {
        // Update existing store with missing indexes
        try {
          const pStore = tx.objectStore(STORES.PIECES_RECHANGE);
          if (!pStore.indexNames.contains("source_document_id")) {
            pStore.createIndex("source_document_id", "source_document_id", { unique: false });
          }
        } catch (e) {
          console.warn("Could not create index on existing PIECES_RECHANGE store");
        }
      }
    };
  });
...
export class ClientStorage {
  ...
  // Equipements
  async addEquipement(equipement: Equipement): Promise<void> {
    return addItem(STORES.EQUIPEMENTS, equipement);
  }

  async deleteEquipement(id: string): Promise<void> {
    return deleteItem(STORES.EQUIPEMENTS, id);
  }

  async getEquipements(userId: string): Promise<Equipement[]> {
    return getItemsByIndex<Equipement>(STORES.EQUIPEMENTS, "userId", userId);
  }

  async getEquipementsByDocument(documentId: string): Promise<Equipement[]> {
    return getItemsByIndex<Equipement>(STORES.EQUIPEMENTS, "source_document_id", documentId);
  }

  async deleteEquipementsByDocument(documentId: string): Promise<void> {
    const items = await this.getEquipementsByDocument(documentId);
    for (const item of items) {
      await this.deleteEquipement(item.id);
    }
  }

  // Maintenance
  async addMaintenance(maintenance: Maintenance): Promise<void> {
    return addItem(STORES.MAINTENANCE, maintenance);
  }

  async deleteMaintenance(id: string): Promise<void> {
    return deleteItem(STORES.MAINTENANCE, id);
  }

  async getMaintenances(userId: string): Promise<Maintenance[]> {
    return getItemsByIndex<Maintenance>(STORES.MAINTENANCE, "userId", userId);
  }

  async getMaintenancesByDocument(documentId: string): Promise<Maintenance[]> {
    return getItemsByIndex<Maintenance>(STORES.MAINTENANCE, "source_document_id", documentId);
  }

  async deleteMaintenancesByDocument(documentId: string): Promise<void> {
    const items = await this.getMaintenancesByDocument(documentId);
    for (const item of items) {
  ...
  // Pieces de rechange
  async addPieceRechange(piece: PieceRechange): Promise<void> {
    return addItem(STORES.PIECES_RECHANGE, piece);
  }

  async deletePieceRechange(id: string): Promise<void> {
    return deleteItem(STORES.PIECES_RECHANGE, id);
  }

  async getPiecesRechange(userId: string): Promise<PieceRechange[]> {
    return getItemsByIndex<PieceRechange>(STORES.PIECES_RECHANGE, "userId", userId);
  }

  async getPiecesRechangeByDocument(documentId: string): Promise<PieceRechange[]> {
    return getItemsByIndex<PieceRechange>(STORES.PIECES_RECHANGE, "source_document_id", documentId);
  }

  async deletePiecesRechangeByDocument(documentId: string): Promise<void> {
    const items = await this.getPiecesRechangeByDocument(documentId);
    for (const item of items) {
      await this.deletePieceRechange(item.id);
    }
  }
}
